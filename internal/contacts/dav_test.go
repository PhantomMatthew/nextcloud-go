package contacts

import (
	"bytes"
	"context"
	"errors"
	"io"
	"log/slog"
	"net/http"
	"testing"
	"time"

	"github.com/PhantomMatthew/nextcloud-go/internal/database"
	"github.com/PhantomMatthew/nextcloud-go/internal/migrations"
	"github.com/PhantomMatthew/nextcloud-go/internal/users"
	"github.com/PhantomMatthew/nextcloud-go/internal/webdav"
)

func TestContactsDAV_RoundTrip(t *testing.T) {
	ctx := context.Background()
	db, err := database.Open(ctx, database.Config{
		Driver: database.DialectSQLite,
		DSN:    "file:" + t.Name() + "?mode=memory&cache=shared",
	})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = db.Close() })
	std, ok := database.Unwrap(db)
	if !ok {
		t.Fatal("unwrap")
	}
	if _, err := migrations.Up(ctx, std, database.DialectSQLite, slog.New(slog.DiscardHandler)); err != nil {
		t.Fatal(err)
	}
	us := users.NewSQLStore(db)
	u := &users.User{UID: "alice", DisplayName: "Alice", PasswordHash: "x", Enabled: true}
	if err := us.Create(ctx, u); err != nil {
		t.Fatal(err)
	}
	store := NewSQLStore(db)
	freeze := time.Date(2025, 5, 1, 12, 0, 0, 0, time.UTC)
	store.Clock = func() time.Time { return freeze }
	dav := &DAV{Store: store, Users: us, Clock: func() time.Time { return freeze }}

	home, err := dav.Stat(ctx, "alice", "/")
	if err != nil || !home.IsDir {
		t.Fatalf("home = %+v %v", home, err)
	}
	kids, err := dav.List(ctx, "alice", "/")
	if err != nil || len(kids) != 1 || !kids[0].IsAddressbook {
		t.Fatalf("list home = %v %v", kids, err)
	}

	ent, created, err := dav.Write(ctx, "alice", "/contacts/ncgo-contact-001.vcf", bytes.NewReader([]byte(sampleVCard)), nil)
	if err != nil || !created || ent.ETag == "" {
		t.Fatalf("write = %+v created=%v err=%v", ent, created, err)
	}
	rc, got, err := dav.Read(ctx, "alice", "/contacts/ncgo-contact-001.vcf")
	if err != nil {
		t.Fatal(err)
	}
	body, _ := io.ReadAll(rc)
	_ = rc.Close()
	if string(body) != sampleVCard {
		t.Fatalf("read = %q", body)
	}
	if got.ContentType != "text/vcard; charset=utf-8" {
		t.Errorf("ct = %q", got.ContentType)
	}

	entries, err := dav.Report(ctx, "alice", "/contacts", webdav.ReportRequest{
		Name: "addressbook-query",
		Body: []byte(`<c:addressbook-query xmlns:c="urn:ietf:params:xml:ns:carddav"/>`),
	})
	if err != nil || len(entries) != 1 {
		t.Fatalf("query = %v %v", entries, err)
	}

	mg, err := dav.Report(ctx, "alice", "/contacts", webdav.ReportRequest{
		Name: "addressbook-multiget",
		Body: []byte(`<c:addressbook-multiget xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:carddav"><d:href>/remote.php/dav/addressbooks/users/alice/contacts/ncgo-contact-001.vcf</d:href><d:href>/remote.php/dav/addressbooks/users/alice/contacts/missing.vcf</d:href></c:addressbook-multiget>`),
	})
	if err != nil || len(mg) != 2 {
		t.Fatalf("multiget = %v %v", mg, err)
	}
	if mg[1].Status != http.StatusNotFound {
		t.Errorf("missing status = %d", mg[1].Status)
	}

	if _, err := dav.Report(ctx, "alice", "/contacts/ncgo-contact-001.vcf", webdav.ReportRequest{Name: "addressbook-query"}); !errors.Is(err, webdav.ErrBadRequest) {
		t.Fatalf("object report = %v", err)
	}
	if _, _, err := dav.Move(ctx, "alice", "/a", "alice", "/b", false); !errors.Is(err, webdav.ErrMethodNotAllowed) {
		t.Fatalf("move = %v", err)
	}
	if _, _, err := dav.Copy(ctx, "alice", "/a", "alice", "/b", false, false); !errors.Is(err, webdav.ErrMethodNotAllowed) {
		t.Fatalf("copy = %v", err)
	}
	if err := dav.Remove(ctx, "alice", "/"); !errors.Is(err, webdav.ErrForbidden) {
		t.Fatalf("remove home = %v", err)
	}
	if _, err := dav.Mkdir(ctx, "alice", "/work"); err != nil {
		t.Fatalf("mkdir = %v", err)
	}
	if _, _, err := splitBookPath("/bad/path/too/deep.vcf"); !errors.Is(err, webdav.ErrBadRequest) {
		t.Fatalf("deep path = %v", err)
	}
}
