...
// csShare is the CalDAV sharing POST body (calendarserver.org/ns share).
type csShare struct {
...
// Share implements webdav.ShareFS: POST cs:share on an own calendar.
...
func (d *DAV) Share(ctx context.Context, user, p string, body []byte) error {
	...
	for _, set := range req.Sets {
		...
		if err := d.Store.UpsertCalendarShare(ctx, rc.cal.ID, target.ID, access); err != nil {
			return mapErr(err)
		}
	}
	for _, rm := range req.Removes {
		...
		if err := d.Store.DeleteCalendarShare(ctx, rc.cal.ID, target.ID); err != nil && !errors.Is(err, ErrNotFound) {
			return mapErr(err)
		}
	}
	return nil
