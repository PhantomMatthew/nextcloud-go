...
func TestSQLiteUpDownUp(t *testing.T) {
	...
	want := []string{
		...
		"notifications", "activities", "ocm_incoming", "calendar_shares",
	}
	...
	var ocmName string
	err = db.QueryRow(ctx, `SELECT name FROM sqlite_master WHERE type='table' AND name=?`, "calendar_shares").Scan(&ocmName)
	if err == nil {
		t.Error("table calendar_shares still present after down to v13")
	}
	var notifName string
	if err := db.QueryRow(ctx, `SELECT name FROM sqlite_master WHERE type='table' AND name=?`, "notifications").Scan(&notifName); err != nil {
		t.Errorf("table notifications missing after down to v13: %v", err)
	}
